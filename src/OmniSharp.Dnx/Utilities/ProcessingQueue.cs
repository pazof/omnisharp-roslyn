...
namespace OmniSharp
{
    public class ProcessingQueue
    {
        ...
        public void Start()
        {
            Logger.LogInformation("[ProcessingQueue]: Start()");
        ...
        public void Post(Message message)
        {
            lock (_writer)
            {
                ...
                {
                    Logger.LogInformation(string.Format("[ProcessingQueue]: Post({0})", message));
                }
        ...
        private void ReceiveMessages()
        {
            while (true)
            {
                try
                {
                    ...
                    if (Logger.IsEnabled(LogLevel.Information))
                    {
                        Logger.LogInformation(string.Format("[ProcessingQueue]: Receive ({0})", message));
                    }
