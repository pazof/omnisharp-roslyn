...
namespace OmniSharp.Dnx
{
    public class DnxPaths
    {
        ...
        string _globalJsonFileName = "global.json";

        private DnxRuntimePathResult GetRuntimePath()
        {
            var root = _env.TargetDirectory;
            var globalJson = (root==null) ? _globalJsonFileName  : Path.Combine(root, _globalJsonFileName) ;
