...
{
    [ExportProjectSystem(ProjectSystemNames.DnxProjectSystem), Shared]
    public class DnxProjectSystem : IProjectSystem
    {
                ...
                _context.Connection.OnReceive += m =>
                {
                    ...
                    else if (m.MessageType == "CompilerOptions")
                    {
                        ...
                        var csharpOptions = new CSharpCompilationOptions(
                                ...
                                platform: (Microsoft.CodeAnalysis.Platform)options.Platform,
        ...
        private void WatchProject(string projectFile)
        {
            ...
            _watcher.Watch(projectFile, (path, chtype) => TriggerDependeees(path, "FilesChanged"));

            // When the project.lock.json file changes, refresh dependencies
            var lockFile = Path.ChangeExtension(projectFile, "lock.json");

            _watcher.Watch(lockFile, (path, chtype) => TriggerDependeees(projectFile, "RefreshDependencies"));
        }
        ...
        {
            _logger.LogInformation(string.Format("Scanning '{0}' for DNX projects", _env.TargetDirectory));

            var anyProjects = false;

            // Single project in this folder
            var projectInThisFolder = Path.Combine(_env.TargetDirectory, "project.json");
            ...
            else
            {
                ...
                if (_options.Projects != "**/project.json")
                {
                    ...
                    paths = matcher.GetResultsInFullPath(_env.TargetDirectory);
                }
                else
                {
                    paths = _directoryEnumerator.SafeEnumerateFiles(_env.TargetDirectory, "project.json");
                }
        ...
        public Task<object> GetProjectModelAsync(string filePath)
        {
            ...
            if (project != null)
                return Task.FromResult<object>(new DnxProject(GetProject(filePath)));
