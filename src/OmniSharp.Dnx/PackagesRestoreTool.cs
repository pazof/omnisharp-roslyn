...
namespace OmniSharp.Dnx
{
    public class PackagesRestoreTool
    {
        ...
        private int DoRun(Project project, int retry)
        {
            ...
            {
                if (e.Data == null)
                    _logger.LogError($"some null data received ... from {psi.FileName} /{project.Path}");
                else _logger.LogInformation(e.Data);
            ...
            restoreProcess.ErrorDataReceived += (sender, e) =>
            {
                // with null data :
                if (e.Data == null)
                    _logger.LogError($"some null data in error data received ... from {psi.FileName}, HasExited:{((Process)sender).HasExited} /{project.Path}.");
                else
